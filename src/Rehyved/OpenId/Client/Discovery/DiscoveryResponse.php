...
class DiscoveryResponse
{
    ...
    private static function validateEndpoints(DiscoveryResponse $response, DiscoveryPolicy $policy)
    {
        ...
        foreach ($json as $key => $value) {
            if (StringHelper::endsWith($key, "endpoint")
                || StringHelper::equals($key, DiscoveryConstants::JWKS_URI)
                || StringHelper::equals($key, DiscoveryConstants::CHECK_SESSION_IFRAME)
            ) {
                $endpoint = $value;
